...
async function getData() {
    ...
    // });
    // return d;
    var data = await fetch('./전국건강증진센터표준데이터.json')
        .then(function (res) {
            return res.json();
        })
        .then(function (r) {
...
function search() {
    ...
    $('input[name=social]:checked').each(function () {
    ...
    $('.item_short').filter(function () {
        ...
        if (nurse.length != 0 && isShow) {
            //간호사수 상세검색
            if (data_list[idx].간호사수 >= Number(nurse[0])) isShow = true;
            else isShow = false;
        }

        if (social.length != 0 && isShow) {
            //사회복지사 수 상세검색
            if (data_list[idx].사회복지사수 >= Number(social[0])) isShow = true;
            else isShow = false;
        }
    });
}
    ...
    $.each(data_list, function (i, item) {
        $('#section').append(
            "<div class='item_short'><div class='item_image'>" +
                "<img src='https://loremflickr.com/200/200/health?random=" +
                i +
                "'></div>" +
