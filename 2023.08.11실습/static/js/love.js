...
$(async function () {
    data = await getData();
    var y = new Set();
    var oldDay = { y: 0, m: 1, d: 0 }; // 이전 월이 몇월, 몇일인지 기억하는 것
...
function draw_data(ctx, month) {
    $.each(month, function (i, m) {
        //월- 월부터 하는 이유는 그래프의 기준이 월이기 때문이다.
        $.each(year, function (k, y) {
            //년
            ...
            if (month[0] == 12)
                //겨울에는 기온이 "-" 이기문에 계산식 따로"
                ctx.beginPath(); //원 그리기 시작하겠다.
            ctx.arc(); //원그리기
...
function draw_axis(ctx, month) {
    ...
    ctx.lineTo(950, 120);
    ctx.stroke();
    var n = month == 9 ? parseInt(maxmin[10].min) : p;
    for (var i = 0; i < 13; i++) {
