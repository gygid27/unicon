...
async function getData() {
    var data = await fetch(
        'https://api.odcloud.kr/api/15031992/v1/uddi:3c46e4ef-78dc-4da6-97b1-ad226624eff5_201911131644?page=1&perPage=500&serviceKey=LoM8pDBOzZU47Rx2GkSLIBQlP6vQLGt89wxlDXM%2FSETK7fMwT%2FNKEz3KLwB776QY99cXGCahe2x0KHn0IkvX5A%3D%3D'
    )
        .then((res) => res.json())
        .then((r) => r);
    return data.data;
...
$(async function () {
    ...
    fish_amount = Array.from(amount).sort(function (a, b) {
        return a - b;
    });
    var inc = parseInt(fish_amount.length / 5);
    fish_amount = [
        Math.round(fish_amount[inc] / 10) * 10,
        Math.round(fish_amount[inc * 2] / 10) * 10,
        Math.round(fish_amount[inc * 3] / 10) * 10,
        Math.round(fish_amount[inc * 4] / 10) * 10,
    ];
    year = Array.from(y);
